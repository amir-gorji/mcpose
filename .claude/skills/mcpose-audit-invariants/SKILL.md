...
3. **Erasable mode (ADR-0018) adds a THIRD private key class, and default mode must stay byte-identical.** With `AuditOptions.keyStore` set, the per-event AES key derives from a stored, destroyable **subject key** (keyed by `identity.sub`, anonymous events sharing the `anonymous` bucket) instead of `encRoot`, and payload hashes become `HMAC(hashSubkey, stableStringify(payload))` with `hashSubkey = HMAC(subjectKey, 'mcpose/v2/hashkey')` — a NEW label for a new mechanism, additive within v2 and pinned in `chain.test.ts`'s "domain label pinning". Ciphertext layout, AAD, and the event-key label/inputs are unchanged: only the root swaps.
   - Subject keys and hash subkeys are private key material with the signing secret's handling rules: never in an event, manifest, telemetry, or error. The `subkey confidentiality (regression)` block covers them too — keep that test passing.
   - **The load-bearing invariant: with no `keyStore`, behavior is byte-identical to before erasable mode existed** — same preimages, same ciphertexts, same plain `sha256` hashes, and no `erasable` key on the event at all. `erasable.test.ts` pins it. Anything that makes default mode diverge is a format break, not a refactor.
   - The store fetch is a pre-call failure alongside subkey derivation, and it is deliberately NOT cached: a cache would survive a `destroy` and hand a re-appearing subject a stale key.
...
- **In the preimage**: `id, startedAt, endedAt, sessionId?, delegatedFrom?, proxy, kind?, erasable?, identity, tool, duration_ms, outcome, sensitivityTier, rejectionReason?, error?, inputHash, outputHash, replayManifestPosition`. `kind` is `'prompt'` only on a prompt event and absent on a tool call (ADR-0014). `erasable` is `true` only on an event recorded in erasable mode and absent otherwise (ADR-0018); it is optional-covered like `kind`, and covering it is what stops a stored event being reinterpreted under the other hash scheme — adding or stripping it must fail verification at that index. Optional fields are omitted when absent, so adding one is additive within v2 and old events keep their original preimage (ADR-0012). `sensitivityTier` and `proxy` are required on every event, so they are plain always-present fields: adding each one changed every preimage, which is why ADR-0015 and then ADR-0019 amended v2 in place before the format's first release rather than treating them as additive. `proxy` moved from optional to required in #123 precisely because the omission pattern made stripping it from a stored event undetectable: an event that never had one and an event whose one was deleted hashed identically.
- **NOT in the preimage**: the raw/encrypted payloads. They are bound only via `inputHash`/`outputHash`, computed with `stableStringify`, which is total, key-order independent, and never throws. That gap is exactly what makes cryptographic erasure possible without touching the chain (ADR-0018), so it is load-bearing rather than an oversight.
- **`buildEvent` must decide the tier before it hashes.** The fail-closed `low`/`medium`-or-`high` choice happens above `withoutChainHash`, and the event ships the same value it hashed. Moving that decision back below `computeChainHash` would omit the tier from the producer's preimage while the verifier still includes it, and every chain would fail verification.
...
- **`closeSession` drains admitted calls before it seals (#168).** Each call registers in `session.inflight` synchronously, BEFORE the first `await` in `observe` (subkey derivation and the erasable-mode key fetch are awaits too, so registering after them leaves a window), and deregisters in an outer `finally` after its event is pushed and `onEvent` has settled; `sealSession` loops until the map is empty and only then deletes the session and signs. A call captures its `SessionState` once at admission and never re-resolves the id after an await: re-resolving is what produced position-0 orphans with no manifest. Concurrent closes share `session.closing`. The drain is bounded by `closeDrainTimeoutMs` (default 30 s): past it the manifest seals what has landed and every straggler is reported through `onAuditError`, so a never-settling handler cannot hold recorded events unsigned forever. Moving the registration below an await, the deregistration above the push, or the delete above the drain silently reopens the hole; `middleware.http.test.ts` covers DELETE and TTL during a blocked call.
- **A failed close is retryable and idempotent (#169).** After the drain the session leaves `sessions` (no further admission), but its `SealedSession` (the unsigned manifest with a fixed `closedAt`, then the signature once obtained) stays in `sealed` until `onManifest` settles successfully. `closeSession` on a sealed id resumes from the failed step: a signer retry signs the same bytes, a sink retry re-delivers the identical manifest, and concurrent retries share one attempt. Never delete the sealed state on failure, never rebuild `unsigned` on retry (a fresh `closedAt` would mint a second signed artifact for one session), and never leave the session in `sessions` past the drain (a late call would append to a manifest already being signed). `middleware.robustness.test.ts` covers one-shot signer and sink failures.
- **Audit never blocks the call path.** Missing `ctx.identity` degrades to `anonymousIdentity()`; a throwing `onEvent`, a throwing resolver, or unserializable args go to `onAuditError` and must never fail or mask the tool call. The only pre-call failures allowed to fail the call are subkey derivation (no keys ⇒ no audit ⇒ fail fast), which must stay retryable (don't cache the rejected promise), a missing `ctx.proxy` (ADR-0019), and, in erasable mode only, a `SubjectKeyStore.getOrCreate` rejection (ADR-0018) — also retryable, since nothing is cached there at all. `proxy` is a required covered field, so a context without one cannot produce a verifiable event; that is a configuration error, since core stamps `ctx.proxy` on every `ProxyContext` and only a host passing a hand-built context can hit it. Never soften it into a sentinel like `{name: 'unknown'}`: a silently unattributable trail is exactly what #85 and #122 exist to prevent. Never move it after the call either, where it would mask the tool result.
