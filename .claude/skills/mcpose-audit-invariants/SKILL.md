...
- **In the preimage**: `id, startedAt, endedAt, sessionId?, delegatedFrom?, proxy?, identity, tool, duration_ms, outcome, rejectionReason?, error?, inputHash, outputHash, replayManifestPosition`. Optional fields are omitted when absent, so adding one is additive within v2 — old events keep their original preimage (ADR-0012).
