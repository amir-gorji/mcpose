...
- **`closeSession` drains admitted calls before it seals (#168).** Each call increments `session.inflight` synchronously with the session lookup at admission and decrements it in a `finally` after its event is pushed and `onEvent` has settled; `sealSession` loops until the count is zero and only then deletes the session and signs. A call captures its `SessionState` once at admission and never re-resolves the id after the upstream returns: re-resolving is what produced position-0 orphans with no manifest. Concurrent closes share `session.closing`. Moving the decrement above the push, or the delete above the drain, silently reopens the hole; `middleware.http.test.ts` covers DELETE and TTL during a blocked call.
