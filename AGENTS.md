...
=======
mcpose is a pnpm 11 + Turborepo monorepo publishing five ESM-only packages: `packages/core` (`mcpose`, the proxy pipeline), `packages/audit` (`@mcpose/audit`, the tamper-evident audit chain), `packages/policy` (`@mcpose/policy`, the deny-by-default RBAC engine), `packages/testing` (`@mcpose/testing`, compliance assertions), and `packages/otel` (`@mcpose/otel`, the OpenTelemetry span adapter for `onTelemetry`).
>>>>>>> 8bd738f (Add @mcpose/policy, the deny-by-default RBAC engine (#127))
