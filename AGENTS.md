...
mcpose is a pnpm 11 + Turborepo monorepo publishing eight ESM-only packages: `packages/core` (`mcpose`, the proxy pipeline), `packages/audit` (`@mcpose/audit`, the tamper-evident audit chain), `packages/policy` (`@mcpose/policy`, the deny-by-default RBAC engine), `packages/consent` (`@mcpose/consent`, the fail-closed GDPR/CCPA consent gate), `packages/testing` (`@mcpose/testing`, compliance assertions), `packages/otel` (`@mcpose/otel`, the OpenTelemetry span adapter for `onTelemetry`), and `packages/store-redis` plus `packages/store-postgres` (`@mcpose/store-redis` and `@mcpose/store-postgres`, persistent `EventStore` and `SessionRegistry` adapters, so SSE reconnect replay survives a restart).
...
A hold-back also has a cost: when the only fix for an advisory ships on the ignored major, osv-scan goes red on every PR until the migration lands, as it did for vitest in [issue #187](https://github.com/amir-gorji/mcpose/issues/187).
