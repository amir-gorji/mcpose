...
- **`@mcpose/audit`** — `closeSession` builds the Merkle tree once and derives the manifest root and every event proof from those shared layers, so sealing a session hashes each tree node once instead of rebuilding the tree per event. Roots, proofs, and the signed payload are byte-identical to before (#174).
